...
For declaring a function, we use the keyword \lstinline|def|:
...
For declaring loops, we use the keywords \lstinline|while| or \lstinline|for| :
\begin{lstlisting}
while condition :
    <statement>
# or
...
Duck typing is useful for defining new classes that can take place in an existing system without having to implement or inherit useless methods from another class. For example, in Python, if you have a library that asks a “File” object as parameter, you can pass a string via the  “cStringIO” \todo{c} class. This class will mimic the behavior of the File class. The name of the concept refers to the duck test, attributed to James Whitcomb Riley, which may be phrased as follows: “When I see a bird that walks like a duck and swims like a duck and quacks like a duck, I call that bird a duck.”

Although Python is duck-typed\todo{Omg so much ducks}, it is possible to check the type of a class via the built-in function \lstinline|type|.
...
As said before, in Python, everything is object and all objects are ``first class''.
...
A lot of things show that Python can do meta-programming.
However, this is not always easy and/or readable.
%As only a subset of what we can do with meta-programming is of interest.
...
If any instantiation exists, the metaclass \lstinline|Singleton| will not create a new object but will bind the existing one to the supposed new one.
...
Limitations of meta-programming in Python are located in everything related to the primary types and to the primary built-in functions. For example, we cannot add a method of the type ``type'', ``object'', ``list'', ``dict'', etc. We can redefine some of these primary attributes, but this might have no effect. For example, redefining ``type'' or ``object'' to ``None'' will have no impact on the creation of new class or object. This is a clear limitation of python as we can alter the global system and this doesn't have any impact.
...
We use the property \lstinline|__proto__| to access the prototype of an object. \todo{Rather use the 'prototype object' term?}
...
true
\end{lstlisting}
\todo[inline]{Use Custom.prototype.constructor.toString() ?}
...
Both Javascript and Python, are duck-typed languages. \todo{dynamically typed languages?}
...
But that will not be enough \todo{Not enough ?} as JavaScript rely on Prototype-chaining to find method name.
So, you will have to explore the attribute \lstinline|__proto__| of \lstinline|__proto__|, and so on until there is no more \lstinline|__proto__| defined.
\setpy
As Python rely on classes, the langage has a clean \todo{simpler?} inheritance system.
By calling \lstinline|dir(objectToAnalyse)|, all attributes that the object inherits are retrieved by using the keys in the \lstinline|__dict__| attribute of the object.
This is not possible in JavaScript as there is no class and no inheritance. \todo{Wtf no inheritance in JavaScript ?!}
