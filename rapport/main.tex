...
The resulting list is not necessarily complete, and may be inaccurate. As this built-in function will rely on the keys in \lstinline|__dict__| and on the method \lstinline|__getattr__()| of the object, we can define (or change at runtime) these attributes.

We can inspect an object with \lstinline|dir|, but we can also inspect a frame at runtime. For doing so, we can use built-in function \lstinline|locals()| and \lstinline|globals()|.
...
\textbf{Introspection}
As we have seen, we can easily inspect and access representation of an object in Python with the method \lstinline|dir()|. We can also inspect locals and globals symbol tables via \lstinline|locals()| and \lstinline|globals()|.

\textbf{Intercession}
We have seen that we can add or replace an attribute (method or variable) in an object (remember that everything is object in python).
